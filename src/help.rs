...
//ssl config
        ...
        (" = ", WHITE, vec![]),
        ("12346", CYAN, vec![]),
        (" # must differ from port; route :80->port, :443->ssl_port\n", GREY, vec![]),
        ...
        (" = ", WHITE, vec![]),
        ("\"serve\"", CYAN, vec![]),
        (" # serve | redirect | https_only\n", GREY, vec![]),
//acme config
        ("acme_enabled", BLUE, vec![]),
        (" = ", WHITE, vec![]),
        ("false", CYAN, vec![]),
        (" # auto Let's Encrypt certs via HTTP-01\n", GREY, vec![]),
        ("acme_domains", BLUE, vec![]),
        (" = ", WHITE, vec![]),
        ("[\"example.com\", \"www.example.com\"]\n", CYAN, vec![]),
        ("acme_contact_email", BLUE, vec![]),
        (" = ", WHITE, vec![]),
        ("\"admin@example.com\"\n", CYAN, vec![]),
        ("acme_production", BLUE, vec![]),
        (" = ", WHITE, vec![]),
        ("false", CYAN, vec![]),
        (" # false = staging; flip to true for real certs\n", GREY, vec![]),
        ("acme_account_path", BLUE, vec![]),
        (" = ", WHITE, vec![]),
        ("\"pems/acme_account.json\"\n", CYAN, vec![]),
        ("acme_renewal_days", BLUE, vec![]),
        (" = ", WHITE, vec![]),
        ("30\n", CYAN, vec![]),
//upload limit
